...
const fetchTopCafes = async () => {
  ...
  const {
    data,
    error
  } = await supabase.from('cafes').select('*').order('rating', {
    ascending: false
  }).limit(4);
  if (error) {
...
const fetchRecentCafes = async () => {
  ...
  const {
    data,
    error
  } = await supabase.from('cafes').select('*').order('created_at', {
    ascending: false
  }).limit(8);
  if (error) {
...
const Index = () => {
  ...
  const cafesWithDistance = allCafes.map(cafe => ({
    ...cafe,
    distance: userLocation ? calculateDistance(userLocation.lat, userLocation.lng, cafe.lat, cafe.lng) : undefined
  }));
  ...
  return <div className="min-h-screen bg-gray-50">
      <div className="bg-primary text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center space-y-6 fade-in">
          ...
          <p className="text-xl md:text-2xl opacity-90 mx-0">Discover cafes, libraries, and community spaces perfect
for  remote work and connection.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-10 space-y-12 pb-16">
        ...
        {highlightedCafes.length > 0 && <section className="space-y-4">
            <h2 className="text-2xl font-bold">Highlights of the Week</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {highlightedCafes.map(cafe => <SpaceCard key={cafe.id} {...cafe} />)}
            </div>
          </section>}
        ...
        <section className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cafesWithDistance.map(cafe => <SpaceCard key={cafe.id} {...cafe} />)}
          </div>
        </section>
      </div>
    </div>;
